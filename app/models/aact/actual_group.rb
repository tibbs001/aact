module Aact
	class ActualGroup < StudyRelationship
		...
		def drop_withdrawals
			@drop_withdrawals ||=Aact::DropWithdrawal.where("nct_id=? and ctgov_group_enumerator=?",nct_id,ctgov_group_enumerator)
		end

		def baseline_measures
			@baseline_measures ||=Aact::BaselineMeasure.where("nct_id=? and ctgov_group_enumerator=?",nct_id,ctgov_group_enumerator)
		end
		...
		def self.summary
			...
			g.milestones.each{|m|
				...
				baseline=Aact::BaselineMeasure.where("nct_id=? and ctgov_group_enumerator=? and title='Number of Participants'",m.nct_id,m.ctgov_group_enumerator)
				baseline.size==1 ? baseline_participant_count=baseline.first.measure_value : baseline_participant_count=nil
				case m.title
					when 'NOT COMPLETED'
						dw=Aact::DropWithdrawal.where("nct_id=? and ctgov_group_id=? and period_id=?",m.nct_id,m.ctgov_group_id,m.period_id)
